...
- All full-page templates include the shared `templates/_nav.html` partial (`{% include "_nav.html" %}`) which renders `<div class="nav-btn-group">` with Home / Embeddings / Jobs links, positioned below the page header. Routes pass `current_page` (`"home"` / `"embeddings"` / `"jobs"` / `""`) to control which button renders as active (`nav-btn--active` span) vs a plain link. Chunks passes `""` — no item is highlighted.

...
Walks ``ListRecords`` pages from ``export.arxiv.org/oai2`` with the
``arXiv`` metadata prefix, caches every raw response to disk so re-parses
during schema iteration don't re-hit the network, and yields one parsed
dict per non-deleted record. The dict shape matches the column names in
``arxiv.schema.papers`` (plus an ``oai_datestamp`` lifted from the record
header so the embed pipeline can detect "this paper changed").

arXiv asks bulk harvesters for ≥ 3 s between requests and a contact
``mailto:`` in the User-Agent — both are honored here.

See https://www.openarchives.org/OAI/openarchivesprotocol.html for the
protocol and https://info.arxiv.org/help/oa/index.html for the arXiv
...
class OAIError(RuntimeError):
    ...
    def __init__(self, code: str, message: str = "") -> None:
        super().__init__(f"OAI-PMH error {code}: {message}" if message else f"OAI-PMH error {code}")
        self.code = code
        self.message = message


def cache_filename(params: dict[str, str]) -> str:
    """Canonical cache-file name for a given OAI request.

    Hashes the params in sorted order so the key is stable regardless of
    dict insertion order or URL-encoding choices.
    """
...
def _parse_authors(arxiv_el: ET.Element) -> list[str]:
    authors_el = arxiv_el.find("arXiv:authors", NS)
    if authors_el is None:
        return []
    out: list[str] = []
    for author in authors_el.findall("arXiv:author", NS):
        keyname_el = author.find("arXiv:keyname", NS)
        forenames_el = author.find("arXiv:forenames", NS)
        keyname = (keyname_el.text or "").strip() if keyname_el is not None and keyname_el.text else ""
        forenames = (forenames_el.text or "").strip() if forenames_el is not None and forenames_el.text else ""
